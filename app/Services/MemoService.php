...
final class MemoService
{
    ...
    public function pageData(int $userId, mixed $type, mixed $trash, mixed $query): array
    {
        ...
        if ($isTrash) {
            $cutoffUtc = (new DateTimeImmutable('now', new DateTimeZone('UTC')))
                ->modify('-30 days')
                ->format('Y-m-d H:i:s');
            $this->memoRepository->purgeExpiredTrash($userId, $cutoffUtc);
        }
        $items = $isTrash
    ...
    {
        return $this->memoRepository->emptyTrash($userId);
    }
    ...
    private function formatMemo(array $memo): array
    {
        ...
        $memo['expiresLabel'] = empty($memo['deleted_at'])
            ? null
            : $this->formatDate((new DateTimeImmutable((string) $memo['deleted_at'], new DateTimeZone('UTC')))
                ->modify('+30 days')
                ->format('Y-m-d H:i:s'));
    ...
    {
        $utcDate = DateTimeImmutable::createFromFormat('!Y-m-d H:i:s', $value, new DateTimeZone('UTC'));
        if (!$utcDate instanceof DateTimeImmutable) {
            return $value;
        }

        return $utcDate->setTimezone(new DateTimeZone('Asia/Seoul'))->format('Y.m.d H:i');
    }
}
